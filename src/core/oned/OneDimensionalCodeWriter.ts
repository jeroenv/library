...
export default abstract class OneDimensionalCodeWriter implements Writer {
  private static /*final*/ NUMERIC = /[0-9]+/; //Pattern.compile("[0-9]+")
   ...
   */
  public abstract encodeContents(
    contents: string,
  ...
  private static renderResult(
    ...
    const /*int*/ leftPadding = Math.floor(
   ...
   */
  protected static /*void*/ checkNumeric(contents: string) {
   ...
   */
  protected static /*int*/ appendPattern(
    target: boolean[],
    /*int*/ pos: number,
    /*int[]*/ pattern: number[],
    startColor: boolean
