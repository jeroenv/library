...
export default /*public final*/ class PDF417 {
   ...
   */
  private static /*final int[][]*/ CODEWORD_TABLE: number[][] = [
    [
  ...
  private static /*final float*/ PREFERRED_RATIO = 3.0 /*f*/;
  private static /*final float*/ DEFAULT_MODULE_WIDTH = 0.357 /*f*/; //1px in mm
  private static /*final float*/ HEIGHT = 2.0 /*f*/; //mm

  private barcodeMatrix: BarcodeMatrix;
  private compact: boolean;
  private compaction: Compaction;
  private encoding: Charset;
  private /*int*/ minCols: number;
  private /*int*/ maxCols: number;
  private /*int*/ maxRows: number;
  private /*int*/ minRows: number;
   ...
   */
  private static /*int*/ calculateNumberOfRows(
    /*int*/ m: number,
    /*int*/ k: number,
    /*int*/ c: number
  ) {
   ...
   */
  private static /*int*/ getNumberOfPadCodewords(
    /*int*/ m: number,
    /*int*/ k: number,
    /*int*/ c: number,
    /*int*/ r: number
  ...
  private /*void*/ encodeLowLevel(
    /*CharSequence*/ fullCodewords: string,
    /*int*/ c: number,
    /*int*/ r: number,
    ...
    for (let y = 0; y < r; y++) {
      ...
      for (let x = 0; x < c; x++) {
        pattern =
          PDF417.CODEWORD_TABLE[cluster][
            fullCodewords.charAt(idx).charCodeAt(0)
          ];
        PDF417.encodeChar(pattern, 17, logic.getCurrentRow());
        idx++;
      }
  ...
  public /*void*/ generateBarcodeLogic(
    ...
    const highLevel = PDF417HighLevelEncoder.encodeHighLevel(
      msg,
      this.compaction,
      this.encoding,
      autoECI
    ...
    const /*int[]*/ dimension = this.determineDimensions(
        sourceCodeWords,
    ...
    const pad = PDF417.getNumberOfPadCodewords(
      sourceCodeWords,
      errorCorrectionCodeWords,
      cols,
      rows
    ...
    const ec = PDF417ErrorCorrection.generateErrorCorrection(
      dataCodewords,
      errorCorrectionLevel
    );
  ...
  private /*int[]*/ determineDimensions(
    /*int*/ sourceCodeWords: number,
    /*int*/ errorCorrectionCodeWords: number
  ) /*throws WriterException*/ {
    let /*float*/ ratio = 0.0; /*f*/
    let /*int[]*/ dimension: number[] = null;

    for (let cols = this.minCols; cols <= this.maxCols; cols++) {
      const /*int*/ rows = PDF417.calculateNumberOfRows(
          sourceCodeWords,
          errorCorrectionCodeWords,
          cols
        );
    ...
    if (dimension == null) {
      const rows = PDF417.calculateNumberOfRows(
        sourceCodeWords,
        errorCorrectionCodeWords,
        this.minCols
      );
   ...
   */
  public /*void*/ setCompact(compact: boolean) {
    this.compact = compact;
  }
   ...
   */
  public /*void*/ setEncoding(encoding: Charset) {
