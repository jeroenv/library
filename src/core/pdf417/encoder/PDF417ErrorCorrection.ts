 ...
 * chapter 4.10.
 */
export default /*final*/ class PDF417ErrorCorrection {
  /**
   * Tables of coefficients for calculating error correction words
   * (see annex F, ISO/IEC 15438:2001(E))
   */
  private static /*final int[][]*/ EC_COEFFICIENTS: number[][] = [
    [27, 917],
    [522, 568, 723, 809],
   ...
   */
  static /*int*/ getErrorCorrectionCodewordCount(
    /*int*/ errorCorrectionLevel: number
  ) {
   ...
   */
  static /*int*/ getRecommendedMinimumErrorCorrectionLevel(
    /*int*/ n: number
  ...
  static /*String*/ generateErrorCorrection(
    /*CharSequence*/ dataCodewords: string,
    /*int*/ errorCorrectionLevel: number
  ) {
    const k = this.getErrorCorrectionCodewordCount(errorCorrectionLevel);
    const e = new Array<char>(k); /*new char[k]*/
    const sld = dataCodewords.length;
    for (let i = 0; i < sld; i++) {
      let t1 =
        (dataCodewords.charAt(i).charCodeAt(0) + (e[e.length - 1] || 0)) % 929;
      let t2;
      let t3;
      for (let j = k - 1; j >= 1; j--) {
        t2 =
