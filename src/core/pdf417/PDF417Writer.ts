 ...
 */
export default /*public final*/ class PDF417Writer implements Writer {
  /**
   * default white space (margin) around the code
   */
  private static /*final*/ WHITE_SPACE: /*int*/ number = 30;

  /**
   * default error correction level
   */
  private static /*final*/ DEFAULT_ERROR_CORRECTION_LEVEL: /*int*/ number = 2;
  ...
  public encode(
    ...
    if (hints != null) {
      ...
      autoECI =
        ...
        Boolean.parseBoolean(
          hints.get(EncodeHintType.PDF417_AUTO_ECI).toString()
        );
    }

    return PDF417Writer.bitMatrixFromEncoder(
      encoder,
      contents,
  ...
  private static bitMatrixFromBitArray(
    ...
    const output = new BitMatrix(
      input[0].length + 2 * margin,
      input.length + 2 * margin
    );
    output.clear();
    for (
      let y = 0, yOutput = output.getHeight() - margin - 1;
      y < input.length;
      y++, yOutput--
    ) {
  ...
  private static rotateArray(bitarray: Uint8Array[]): Uint8Array[] {
    let temp = new Array<Uint8Array>(bitarray[0].length); // new byte[bitarray[0].length][bitarray.length];
