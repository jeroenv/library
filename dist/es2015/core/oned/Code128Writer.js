...
export default /*public final*/ class Code128Writer extends OneDimensionalCodeWriter {
    ...
    static /*int*/ check(contents, hints) {
        const /*int*/ length = contents.length;
        // Check length
        if (length < 1 || length > 80) {
            throw new IllegalArgumentException('Contents length should be between 1 and 80 characters, but got ' +
                length);
        }
        ...
        for (let i = 0; i < length; i++) {
            const c = contents.charAt(i).charCodeAt(0);
            ...
            switch (forcedCodeSet) {
                case Code128Writer.CODE_CODE_A:
                    ...
                    if (c > 95 && c <= 127) {
                        throw new IllegalArgumentException('Bad character in input for forced code set A: ASCII value=' +
                            /*(int)*/ c);
                    }
                    break;
                case Code128Writer.CODE_CODE_B:
                    ...
                    if (c <= 32) {
                        throw new IllegalArgumentException('Bad character in input for forced code set B: ASCII value=' +
                            /*(int)*/ c);
                    }
                    break;
                case Code128Writer.CODE_CODE_C:
                    ...
                    if (c < 48 ||
                        ...
                        throw new IllegalArgumentException('Bad character in input for forced code set C: ASCII value=' +
                            /*(int)*/ c);
                    }
                    break;
            }
    ...
    static encodeFast(contents, 
    /*int*/ forcedCodeSet) {
        const /*int*/ length = contents.length;
        const /*Collection<int[]>*/ patterns = new Array(); // temporary storage for patterns
        let /*int*/ checkSum = 0;
...
/*private static final*/ class MinimalEncoder {
    encode(contents) {
        ...
        const /*Collection<int[]>*/ patterns = new Array();
        ...
        for (let i = 0; i < length; i++) {
            ...
            if (charset == Charset.C) {
                if (contents.charAt(i).charCodeAt(0) ==
                    ...
                    MinimalEncoder.addPattern(patterns, Integer.parseInt(contents.substring(i, i + 2)), checkSum, checkWeight, i);
                    if (!(i + 1 < length)) {
    ...
    /*int*/ position) {
        ...
        const /*int*/ mCost = this.memoizedCost[getOrdinal(Charset, charset)][position]; //charset.ordinal()
        if (mCost > 0) {
            return mCost;
        }
        ...
        if (this.canEncode(contents, Charset.C, position)) {
            ...
            let /*int*/ advance = contents.charAt(position).charCodeAt(0) ==
                Code128Writer.ESCAPE_FNC_1.charCodeAt(0)
                ? 1
                : 2;
