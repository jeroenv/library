...
export default /*final*/ class PDF417HighLevelEncoder {
     ...
     */
    static /*String*/ encodeHighLevel(msg, compaction, encoding, autoECI) {
        if (!msg) {
            //msg.isEmpty()
        ...
        switch (compaction) {
            ...
            case Compaction.BYTE:
                if (autoECI) {
                    ...
                    const /*byte[]*/ msgBytes = StringUtils.getBytes(input.toString(), encoding);
                ...
                while (p < len) {
                    ...
                    if (n >= 13) {
                        ...
                        if (t >= 5 || n == len) {
                            ...
                            let /*int*/ b = this.determineConsecutiveBinaryCount(input, p, autoECI ? null : encoding);
                            if (b == 0) {
                                b = 1;
                            }
    ...
    /*int*/ initialSubmode) {
        ...
        while (true) {
            if (input.isECI(startpos + idx)) {
                ...
                const ch = input.charAt(startpos + idx);
                switch (submode) {
     ...
     * @param sb        receives the encoded codewords
     */
    static /*void*/ encodeMultiECIBinary(input, 
    /*int*/ startpos, 
    /*int*/ count, 
    /*int*/ startmode, sb) {
    ...
    static /*void*/ encodeNumeric(input, 
    /*int*/ startpos, 
    /*int*/ count, sb) {
        let idx = 0;
        const tmp = new StringBuilder();
        ...
        while (idx < count) {
            ...
            const part = '1' + input.subSequence(startpos + idx, startpos + idx + len);
    ...
    static /*boolean*/ isAlphaUpper(ch) {
        return (ch === ' '.charCodeAt(0) ||
            (ch >= 'A'.charCodeAt(0) && ch <= 'Z'.charCodeAt(0)));
    }
    static /*boolean*/ isAlphaLower(ch) {
        return (ch === ' '.charCodeAt(0) ||
            (ch >= 'a'.charCodeAt(0) && ch <= 'z'.charCodeAt(0)));
    }
    static /*boolean*/ isMixed(ch) {
        return PDF417HighLevelEncoder.MIXED[ch] != -1;
    }
    static /*boolean*/ isPunctuation(ch) {
        return PDF417HighLevelEncoder.PUNCTUATION[ch] != -1;
    }
    static /*boolean*/ isText(ch) {
        return (ch === '\t'.charCodeAt(0) ||
            ch === '\n'.charCodeAt(0) ||
            ch === '\r'.charCodeAt(0) ||
            (ch >= 32 && ch <= 126));
     ...
     */
    static /*int*/ determineConsecutiveDigitCount(input, 
    /*int*/ startpos) {
        let count = 0;
     ...
     * @return the requested character count
     */
    static /*int*/ determineConsecutiveTextCount(input, 
    /*int*/ startpos) {
        const /*final int*/ len = input.length();
        let idx = startpos;
        while (idx < len) {
     ...
     * @return the requested character count
     */
    static /*int*/ determineConsecutiveBinaryCount(input, 
    /*int*/ startpos, encoding) {
        const /*CharsetEncoder*/ encoder = encoding === null ? null : new CharsetEncoder(encoding);
        const len = input.length();
        let idx = startpos;
        while (idx < len) {
            let numericCount = 0;
...
(() => {
    ...
    for (let i = 0; i < PDF417HighLevelEncoder.TEXT_PUNCTUATION_RAW.length; i++) {
