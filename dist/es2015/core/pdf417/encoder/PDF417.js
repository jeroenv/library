 ...
 */
export default /*public final*/ class PDF417 {
     ...
     */
    static /*int*/ calculateNumberOfRows(
    /*int*/ m, 
    /*int*/ k, 
    /*int*/ c) {
     ...
     */
    static /*int*/ getNumberOfPadCodewords(
    /*int*/ m, 
    /*int*/ k, 
    /*int*/ c, 
    /*int*/ r) {
    ...
    /*int*/ len, logic) {
        let map = 1 << (len - 1);
        let last = (pattern & map) != 0; //Initialize to inverse of first bit
        let width = 0;
        for (let i = 0; i < len; i++) {
            const black = (pattern & map) != 0;
            if (last == black) {
                width++;
            }
            else {
    ...
    /*int*/ errorCorrectionLevel, logic) {
        ...
        for (let y = 0; y < r; y++) {
            ...
            let pattern = PDF417.CODEWORD_TABLE[cluster][left];
    ...
    /*String*/ msg, 
    /*int*/ errorCorrectionLevel, autoECI = false) {
        //1. step: High-level encoding
        let errorCorrectionCodeWords = PDF417ErrorCorrection.getErrorCorrectionCodewordCount(errorCorrectionLevel);
        const highLevel = PDF417HighLevelEncoder.encodeHighLevel(msg, this.compaction, this.encoding, autoECI);
        const sourceCodeWords = highLevel.length;
        const /*int[]*/ dimension = this.determineDimensions(sourceCodeWords, errorCorrectionCodeWords);
        const cols = dimension[0];
        const rows = dimension[1];
        const pad = PDF417.getNumberOfPadCodewords(sourceCodeWords, errorCorrectionCodeWords, cols, rows);
        ...
        const dataCodewords = sb.toString();
        //3. step: Error correction
        const ec = PDF417ErrorCorrection.generateErrorCorrection(dataCodewords, errorCorrectionLevel);
    ...
    /*int*/ errorCorrectionCodeWords) {
        let /*float*/ ratio = 0.0; /*f*/
        let /*int[]*/ dimension = null;
        for (let cols = this.minCols; cols <= this.maxCols; cols++) {
            const /*int*/ rows = PDF417.calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, cols);
        ...
        if (dimension == null) {
            const rows = PDF417.calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, this.minCols);
            if (rows < this.minRows) {
