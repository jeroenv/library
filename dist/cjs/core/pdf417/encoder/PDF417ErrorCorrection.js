 ...
 * chapter 4.10.
 */
var PDF417ErrorCorrection = /** @class */ (function () {
     ...
     */
    PDF417ErrorCorrection.getErrorCorrectionCodewordCount = function (
    /*int*/ errorCorrectionLevel) {
        if (errorCorrectionLevel < 0 || errorCorrectionLevel > 8) {
            throw new IllegalArgumentException_1.default('Error correction level must be between 0 and 8!');
        }
     ...
     */
    PDF417ErrorCorrection.getRecommendedMinimumErrorCorrectionLevel = function (
    /*int*/ n) {
        if (n <= 0) {
            throw new IllegalArgumentException_1.default('n must be > 0');
        }
        if (n <= 40) {
            return 2;
        }
        ...
        throw new WriterException_1.default('No recommendation possible');
    };
    /**
     ...
     */
    PDF417ErrorCorrection.generateErrorCorrection = function (
    /*CharSequence*/ dataCodewords, 
    /*int*/ errorCorrectionLevel) {
        var k = this.getErrorCorrectionCodewordCount(errorCorrectionLevel);
        var e = new Array(k); /*new char[k]*/
        var sld = dataCodewords.length;
        for (var i = 0; i < sld; i++) {
            var t1 = (dataCodewords.charAt(i).charCodeAt(0) + (e[e.length - 1] || 0)) % 929;
            var t2 = void 0;
            var t3 = void 0;
            for (var j = k - 1; j >= 1; j--) {
                t2 =
                    (t1 *
                        PDF417ErrorCorrection.EC_COEFFICIENTS[errorCorrectionLevel][j]) %
                        929;
                t3 = 929 - t2;
                e[j] = /*(char)*/ ((e[j - 1] || 0) + t3) % 929;
            }
            t2 =
                (t1 * PDF417ErrorCorrection.EC_COEFFICIENTS[errorCorrectionLevel][0]) %
                    929;
            t3 = 929 - t2;
            e[0] = /*(char)*/ t3 % 929;
        }
     ...
     */
    PDF417ErrorCorrection.EC_COEFFICIENTS = [
        [27, 917],
        [522, 568, 723, 809],
        ...
        [
            ...
            201, 488, 502, 648, 733, 717, 83, 404, 97, 280, 771, 840, 629, 4, 381,
            843, 623, 264, 543,
        ],
        [
        ...
        [
            ...
            736, 707, 94, 8, 494, 114, 521, 2, 499, 851, 543, 152, 729, 771, 95, 248,
            361, 578, 323, 856, 797, 289, 51, 684, 466, 533, 820, 669, 45, 902, 452,
            167, 342, 244, 173, 35, 463, 651, 51, 699, 591, 452, 578, 37, 124, 298,
            332, 552, 43, 427, 119, 662, 777, 475, 850, 764, 364, 578, 911, 283, 711,
            472, 420, 245, 288, 594, 394, 511, 327, 589, 777, 699, 688, 43, 408, 842,
            383, 721, 521, 560, 644, 714, 559, 62, 145, 873, 663, 713, 159, 672, 729,
            624, 59, 193, 417, 158, 209, 563, 564, 343, 693, 109, 608, 563, 365, 181,
            772, 677, 310, 248, 353, 708, 410, 579, 870, 617, 841, 632, 860, 289, 536,
            35, 777, 618, 586, 424, 833, 77, 597, 346, 269, 757, 632, 695, 751, 331,
            247, 184, 45, 787, 680, 18, 66, 407, 369, 54, 492, 228, 613, 830, 922,
            437, 519, 644, 905, 789, 420, 305, 441, 207, 300, 892, 827, 141, 537, 381,
            662, 513, 56, 252, 341, 242, 797, 838, 837, 720, 224, 307, 631, 61, 87,
            560, 310, 756, 665, 397, 808, 851, 309, 473, 795, 378, 31, 647, 915, 459,
            806, 590, 731, 425, 216, 548, 249, 321, 881, 699, 535, 673, 782, 210, 815,
            905, 303, 843, 922, 281, 73, 469, 791, 660, 162, 498, 308, 155, 422, 907,
            817, 187, 62, 16, 425, 535, 336, 286, 437, 375, 273, 610, 296, 183, 923,
            116, 667, 751, 353, 62, 366, 691, 379, 687, 842, 37, 357, 720, 742, 330,
            5, 39, 923, 311, 424, 242, 749, 321, 54, 669, 316, 342, 299, 534, 105,
            667, 488, 640, 672, 576, 540, 316, 486, 721, 610, 46, 656, 447, 171, 616,
            464, 190, 531, 297, 321, 762, 752, 533, 175, 134, 14, 381, 433, 717, 45,
            111, 20, 596, 284, 736, 138, 646, 411, 877, 669, 141, 919, 45, 780, 407,
            164, 332, 899, 165, 726, 600, 325, 498, 655, 357, 752, 768, 223, 849, 647,
            63, 310, 863, 251, 366, 304, 282, 738, 675, 410, 389, 244, 31, 121, 303,
            263,
        ],
    ];
    return PDF417ErrorCorrection;
}());
exports.default = PDF417ErrorCorrection;
//# sourceMappingURL=PDF417ErrorCorrection.js.map