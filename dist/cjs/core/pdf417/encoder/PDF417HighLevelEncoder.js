...
var Compaction_1 = require("./Compaction");
var ECIEncoderSet_1 = require("../../common/ECIEncoderSet");
...
var PDF417HighLevelEncoder = /** @class */ (function () {
    ...
    PDF417HighLevelEncoder.encodeHighLevel = function (msg, compaction, encoding, autoECI) {
        if (!msg) {
            //msg.isEmpty()
            throw new WriterException_1.default('Empty message not allowed');
        }
        if (encoding == null && !autoECI) {
            for (var i = 0; i < msg.length; i++) {
                if (msg.charAt(i).charCodeAt(0) > 255) {
                    throw new WriterException_1.default('Non-encodable character detected: ' +
                        msg.charAt(i) +
                        ' (Unicode: ' +
        ...
        var input;
        if (autoECI) {
            input = new MinimalECIInput_1.MinimalECIInput(msg, encoding, -1);
        }
        else {
                    ...
                    if (n >= 13) {
                        ...
                        if (t >= 5 || n == len) {
                            ...
                            var /*byte[]*/ bytes = autoECI
                                ? null
                                : StringUtils_1.default.getBytes(input.subSequence(p, p + b).toString(), encoding);
                            if (((bytes == null && b == 1) ||
    ...
    /*int*/ count, sb, 
    /*int*/ initialSubmode) {
        var tmp = new StringBuilder_1.default();
    ...
    /*int*/ startpos, encoding) {
        var /*CharsetEncoder*/ encoder = encoding === null ? null : new ECIEncoderSet_1.CharsetEncoder(encoding);
        var len = input.length();
        var idx = startpos;
        while (idx < len) {
            var numericCount = 0;
            ...
            if (encoder != null &&
                !encoder.canEncode(StringUtils_1.default.getCharAt(input.charAt(idx)))) {
    ...
    /*int*/ eci, sb) {
        if (eci >= 0 && eci < 900) {
            ...
            throw new WriterException_1.default('ECI number not in valid range from 0..811799, but was ' + eci);
        }
    };
    /**
    ...
    PDF417HighLevelEncoder.DEFAULT_ENCODING = StandardCharsets_1.default.ISO_8859_1;
    (function () {
        //Construct inverse lookups
        Arrays_1.default.fill(PDF417HighLevelEncoder.MIXED, /*(byte)*/ -1);
        ...
        Arrays_1.default.fill(PDF417HighLevelEncoder.PUNCTUATION, /*(byte)*/ -1);
...
exports.default = PDF417HighLevelEncoder;
