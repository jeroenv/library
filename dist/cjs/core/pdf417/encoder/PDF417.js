...
var BarcodeMatrix_1 = require("./BarcodeMatrix");
var Compaction_1 = require("./Compaction");
var PDF417ErrorCorrection_1 = require("./PDF417ErrorCorrection");
var StringBuilder_1 = require("../../util/StringBuilder");
var PDF417HighLevelEncoder_1 = require("./PDF417HighLevelEncoder");
/*
 ...
 */
var PDF417 = /** @class */ (function () {
    function PDF417(compact) {
        if (compact === void 0) { compact = false; }
        this.compact = compact;
        this.compaction = Compaction_1.default.AUTO;
    ...
    PDF417.prototype.getBarcodeMatrix = function () {
     ...
     */
    PDF417.calculateNumberOfRows = function (
    /*int*/ m, 
    /*int*/ k, 
    /*int*/ c) {
        var r = Math.floor((m + 1 + k) / c + 1);
        if (c * r >= m + 1 + k + c) {
            r--;
        }
        return r;
     ...
     */
    PDF417.getNumberOfPadCodewords = function (
    /*int*/ m, 
    /*int*/ k, 
    /*int*/ c, 
    /*int*/ r) {
    ...
    PDF417.prototype.encodeLowLevel = function (
    /*CharSequence*/ fullCodewords, 
    /*int*/ c, 
    /*int*/ r, 
    /*int*/ errorCorrectionLevel, logic) {
        var idx = 0;
        for (var y = 0; y < r; y++) {
            var cluster = y % 3;
            logic.startRow();
            ...
            var right = void 0;
            if (cluster == 0) {
                left = Math.floor(30 * Math.floor(y / 3) + (r - 1) / 3);
                right = 30 * Math.floor(y / 3) + (c - 1);
            }
            else if (cluster == 1) {
                left =
                    30 * Math.floor(y / 3) + errorCorrectionLevel * 3 + ((r - 1) % 3);
                right = Math.floor(30 * Math.floor(y / 3) + (r - 1) / 3);
            }
            else {
            ...
            var pattern = PDF417.CODEWORD_TABLE[cluster][left];
            PDF417.encodeChar(pattern, 17, logic.getCurrentRow());
            for (var x = 0; x < c; x++) {
                pattern =
                    PDF417.CODEWORD_TABLE[cluster][fullCodewords.charAt(idx).charCodeAt(0)];
                PDF417.encodeChar(pattern, 17, logic.getCurrentRow());
                idx++;
            }
            if (this.compact) {
     ...
     */
    PDF417.prototype.generateBarcodeLogic = function (
    /*String*/ msg, 
    /*int*/ errorCorrectionLevel, autoECI) {
        if (autoECI === void 0) { autoECI = false; }
        //1. step: High-level encoding
        var errorCorrectionCodeWords = PDF417ErrorCorrection_1.default.getErrorCorrectionCodewordCount(errorCorrectionLevel);
        var highLevel = PDF417HighLevelEncoder_1.default.encodeHighLevel(msg, this.compaction, this.encoding, autoECI);
        var sourceCodeWords = highLevel.length;
        var /*int[]*/ dimension = this.determineDimensions(sourceCodeWords, errorCorrectionCodeWords);
        var cols = dimension[0];
        var rows = dimension[1];
        var pad = PDF417.getNumberOfPadCodewords(sourceCodeWords, errorCorrectionCodeWords, cols, rows);
        //2. step: construct data codewords
        if (sourceCodeWords + errorCorrectionCodeWords + 1 > 929) {
            // +1 for symbol length CW
            throw new WriterException_1.default('Encoded message contains too many code words, message too big (' +
                msg.length +
                ' bytes)');
        }
        ...
        var dataCodewords = sb.toString();
        //3. step: Error correction
        var ec = PDF417ErrorCorrection_1.default.generateErrorCorrection(dataCodewords, errorCorrectionLevel);
        //4. step: low-level encoding
        this.barcodeMatrix = new BarcodeMatrix_1.default(rows, cols);
     ...
     */
    PDF417.prototype.determineDimensions = function (
    /*int*/ sourceCodeWords, 
    /*int*/ errorCorrectionCodeWords) {
        var /*float*/ ratio = 0.0; /*f*/
        var /*int[]*/ dimension = null;
        for (var cols = this.minCols; cols <= this.maxCols; cols++) {
            var /*int*/ rows = PDF417.calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, cols);
            ...
            var /*float*/ newRatio = 
            /*(float)*/ ((17 * cols + 69) * PDF417.DEFAULT_MODULE_WIDTH) /
                (rows * PDF417.HEIGHT);
            // ignore if previous ratio is closer to preferred ratio
            if (dimension != null &&
                Math.abs(newRatio - PDF417.PREFERRED_RATIO) >
                    Math.abs(ratio - PDF417.PREFERRED_RATIO)) {
                continue;
            }
            ratio = newRatio;
            dimension = [cols, rows]; //new int[] {cols, rows};
        }
        // Handle case when min values were larger than necessary
        if (dimension == null) {
            var rows = PDF417.calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, this.minCols);
            if (rows < this.minRows) {
                dimension = [this.minCols, this.minRows]; //new int[]{minCols, minRows};
            }
        }
        if (dimension == null) {
            throw new WriterException_1.default('Unable to fit message in columns');
        }
        return dimension;
    };
    /**
     ...
     */
    PDF417.prototype.setDimensions = function (
    /*int*/ maxCols, 
    /*int*/ minCols, 
    /*int*/ maxRows, 
    /*int*/ minRows) {
     ...
     */
    PDF417.prototype.setCompaction = function (compaction) {
        this.compaction = compaction;
    };
    /**
     * @param compact if true, enables compaction
     */
    PDF417.prototype.setCompact = function (compact) {
        this.compact = compact;
    };
    /**
     * @param encoding sets character encoding to use
     */
    PDF417.prototype.setEncoding = function (encoding) {
        this.encoding = encoding;
    };
    /**
     * The start pattern (17 bits)
     */
    PDF417.START_PATTERN = 0x1fea8;
    /**
     * The stop pattern (18 bits)
     */
    PDF417.STOP_PATTERN = 0x3fa29;
    /**
     * The codeword table from the Annex A of ISO/IEC 15438:2001(E).
     */
    PDF417.CODEWORD_TABLE = [
        [
    ...
    PDF417.PREFERRED_RATIO = 3.0 /*f*/;
    PDF417.DEFAULT_MODULE_WIDTH = 0.357 /*f*/; //1px in mm
    PDF417.HEIGHT = 2.0 /*f*/; //mm
    return PDF417;
}());
exports.default = PDF417;
//# sourceMappingURL=PDF417.js.map